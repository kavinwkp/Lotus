...
from lotus.lifelong.models.modules.transformer_modules import SparseMoE2
...
def main():
    ...
    with Timer() as t, VideoWriter(video_folder, args.save_videos) as video_writer:
        ...
        # expert frequency
        for layer in algo.policy.temporal_transformer.layers:
            for module in layer:
                if isinstance(module, SparseMoE2):
                    # print(module.experts_counts)
                    rounded_values = np.round((module.experts_counts / module.experts_counts.sum()).numpy(), 2)
