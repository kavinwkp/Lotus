...
from lotus.lifelong.algos import get_algo_class, get_algo_list, Multitask, DiffusionTask
...
def main(hydra_cfg):
    ...
    n_manip_tasks = benchmark.n_tasks  # libero_spatial:10
    new_task_name = benchmark.new_task_name  # libero_spatial:@default@
    ...
    for i in range(n_manip_tasks):
        ...
        try:
            task_i_dataset, shape_meta = get_dataset(
                ...
                obs_modality=cfg.data.obs.modality,  # {"rgb": ["agentview_rgb","eye_in_hand_rgb"],"depth": [],"low_dim": ["joint_states"]}
    ...
    # datasets = [SequenceVLDataset(manip_datasets[1], task_embs[1], task_ids[1])]
    # datasets = [SequenceVLDataset(manip_datasets[7], task_embs[7], task_ids[7])]
    ...
    # {'ac_dim': 7, 'all_shapes': OrderedDict([('agentview_rgb', [3, 128, 128]), ('eye_in_hand_rgb', [3, 128, 128]), ('gripper_states', [2]), ('joint_states', [7])]),
    ...
    algo = safe_device(DiffusionTask(n_tasks, cfg), cfg.device)

    model_size = sum(p.numel() for p in algo.policy.parameters() if p.requires_grad)
    print(f"[info] Diffusion Policy Model size: {model_size / 1e6:.2f}M")
    ...
    # if cfg.lifelong.algo == "Multitask":
        algo.train()
        # print(algo.policy)
        algo.learn_all_tasks(datasets, benchmark, cfg.use_wandb)
    ...
    #     # for i in range(n_tasks):
        ...
        t0 = time.time()
        s_fwd, l_fwd = algo.learn_one_task(datasets[i], i, benchmark, result_summary, cfg.use_wandb)
